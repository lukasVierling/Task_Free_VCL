...
def evaluate_model(task_idx, model, test_datasets, batch_size,device):
    ...
    for _, test_dataset in enumerate(test_datasets[:task_idx+1]):
        ...
        acc, heads_chosen = perform_predictions(model, test_dataset, batch_size, device)
        accs.append(acc)
        heads_chosen.append(heads_chosen)
        print("test acc after additional finetuning: ",accs[-1])
        print("Heads chosen statistics:",heads_chosen[-1])
        #reset the finetuning
    model.activate_head(prev_head)
    mean_accs = sum(accs)/len(accs)
    return mean_accs, heads_chosen
...
def perform_predictions(model, curr_test_dataset,batch_size,device, num_samples=100):
    ...
    with torch.no_grad():
        for x,y in tqdm(data_loader,desc="Testing Performance"):
            ...
            heads_chosen[best_head] += batch_size
            probs.append(preds)
...
def coreset_vcl(model, train_datasets, test_datasets, batch_size, epochs, lr, device="cpu", baseline_window_size=25, current_window_size=1, c=3):
    '''
    Input:
    ...
    for i in tqdm(range(T), desc="Training on tasks..."):
        ...
        for epoch in tqdm(range(epochs), desc="Training"):
            for batch_idx,(x,y) in tqdm(enumerate(data_loader), desc=f"Training in Epoch: {epoch}"): 
                ...
                if prior is not None:
                    rhs = kl_div_gaussian(model.get_var_dist(detach=False), prior)
                ...
                model.eval()
                MI = model.get_MI(x) #TODO consider evaluating before backprop
                mean_MI = MI.mean()
                ...
                if len(current_mi) == current_window_size:
                    ...
                    if len(baseline_mi) == baseline_window_size:
                        ...
                        if current_mean > baseline_mean + c * baseline_std:
                            print("Found a task switch at batch idx: {batch_idx} in epoch: {epoch}")
                            model.add_head()
        ...
        acc = evaluate_model(i, model, test_datasets, batch_size, device)
