...
def update_final_var_dist_and_test(task_idx, model, prior ,prior_heads,prior_encoders, curr_coreset, test_datasets, batch_size, epochs, lr, device):
    # get a new distribution q
    accs = []
    ...
    if curr_coreset is not None:
        for idx, coreset in enumerate(curr_coreset[:task_idx+1]):
            ...
            print("test acc before additional finetuning: ",perform_generations(model, test_datasets[idx], batch_size, device))
            minimize_KL(model, prior, coreset ,batch_size, epochs, lr, device)
            #obtained finetuned model for dataset head_idx
            #test on the test dataset
            accs.append(perform_generations(model, test_datasets[idx], batch_size, device))
        ...
        for idx, test_dataset in enumerate(test_datasets[:task_idx+1]):
            ...
            model.activate_encoder(idx)
            #test on the test dataset
            accs.append(perform_generations(model, test_dataset,batch_size, device))
            print("test acc after additional finetuning: ",accs[-1])
            #reset the finetuning
...
def perform_generations(model, curr_test_dataset, batch_size,device):
    ...
    if filename is None:
        ...
        filename = f"generation_{timestamp}.png"
...
def coreset_vcl(model, train_datasets, test_datasets, batch_size, epochs, lr, coreset_size=0, coreset_heuristic="random", device="cpu"):
    ...
    for i in tqdm(range(T), desc="Training on tasks..."):
        ...
        #acc = update_final_var_dist_and_test(i, model, prior, heads, encoders, curr_coresets, test_datasets, batch_size, epochs, lr, device)
